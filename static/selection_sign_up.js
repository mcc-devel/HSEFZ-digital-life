...
function convert_form_data_to_json(data) {
    ...
    for (c of data['data']) {
        ...
        if (c['is_favorite'] == 1) {
            favorite_content = "<button class='btn favorite-btn-cancel cancel-favorite' id='{0}'>取消收藏</button>".format(class_id);
        }

        else {
            favorite_content = "<button class='btn favorite-btn add-favorite' id='{0}'>收藏</button>".format(class_id);
        }
...
function get_favorite_and_refresh(data) {
    $.ajax({
        url: favorite_url,
        data: JSON.stringify(data),
        dataType: 'json',
...
function add_favorite() {
    const btn = $(this);
    btn.html('<div style="display: flex;"><div class="spinner-border text-dark"></div></div>');
    btn.prop('disabled', true);
    // get_result_and_refresh({ class_id: parseInt($(this).attr('id')), type: 'register' });
    get_favorite_and_refresh({
...
function remove_favorite() {
    const btn = $(this);
    btn.html('<div style="display: flex;"><div class="spinner-border text-info"></div></div>');
    btn.prop('disabled', true);
